...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	try {
		// Ambil daftar kawasan yang tersedia untuk dropdown filter
		let daftarKawasan: string[] = [];
		const companyDoc = await ganoAIDbAdmin.collection('company').doc(companyIdToLoad).get();
		...
		querySnapshot.forEach((doc) => {
			const data = doc.data();
			const createdDateISO = serializeAdminTimestamp(
				data.date?.createdDate as admin.firestore.Timestamp | undefined
			);
			const updatedDateISO = serializeAdminTimestamp(
		...
		return {
			...
			daftarKawasan: daftarKawasan, // Kirim daftar kawasan ke UI
			filters: {
				status: filterStatus,
				startDate: startDateStr,
				endDate: endDateStr,
				kawasan: filterKawasan // Kirim filter aktif ke UI
			},
