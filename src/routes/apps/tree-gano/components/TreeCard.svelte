<script lang="ts">
	...
	function getStatusBadge(status: string | undefined): { color: string; text: string; icon: string } {
		...
		if (s === 'sick')
			return { color: 'danger', text: 'Sakit (Ganoderma)', icon: 'mdi:virus-off-outline' };
		if (s === 'recovered')
			return { color: 'success', text: 'Sudah Pulih', icon: 'healthicons:health-outline-24px' };
		if (s === 'maintenance')
			return { color: 'info', text: 'Dalam Perawatan', icon: 'mdi:tools' };
...
<Card class="h-100 d-flex flex-column {className || ''} {$$props.class || ''}">
	...
	<CardBody class="d-flex flex-column flex-grow-1 p-3">
		<h5 class="card-title fs-16 mb-1 text-truncate">
			...
			<small class="text-muted fw-normal">({tree.id?.substring(0, 8) || 'N/A'})</small>
		</h5>
		...
		<div class="mt-auto d-flex gap-2">
			{#if tree.img}
				<Button
					...
					color="light"
					outline
					on:click={(event) => onViewPhoto(event, tree)}
					class="flex-grow-1"
				>
			...
			<Button
				...
				color="primary"
				outline
				on:click={(event) => onViewTimeline(event, tree)}
				class="flex-grow-1"
			>
