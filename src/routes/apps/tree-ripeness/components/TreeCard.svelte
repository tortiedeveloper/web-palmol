<script lang="ts">
	...
	import type { Tree, FruitCounts } from '$lib/types';
	...
	function getRipenessStatusInfo(fruitCounts?: FruitCounts): { color: string; text: string; icon: string } {
		...
		if (fruitCounts.terlaluMatang > 0) return { color: 'warning', text: 'Ada Terlalu Matang', icon: 'mdi:fruit-cherries-off' };
		if (fruitCounts.matang > 0) return { color: 'success', text: 'Siap Panen', icon: 'mdi:fruit-cherries' };
		if (fruitCounts.belumMatang > 0) return { color: 'info', text: 'Belum Matang', icon: 'mdi:fruit-cherries-outline' };
...
<Card class="h-100 d-flex flex-column {className || ''} {$$props.class || ''}">
	...
	<CardBody class="d-flex flex-column flex-grow-1 p-3">
		...
		<p class="text-muted fw-normal mb-1" style="font-size: 0.75rem;">
			ID: {tree.id?.substring(0, 8) || 'N/A'}
		</p>
        ...
        {#if tree.kawasan && tree.kawasan !== 'N/A'}
            <p class="card-text text-muted mb-1" style="font-size: 0.8rem;">
                <Icon icon="mdi:map-marker-outline" class="me-1" style="vertical-align: -2px;"/>
                Kawasan: {tree.kawasan}
            </p>
        {/if}
